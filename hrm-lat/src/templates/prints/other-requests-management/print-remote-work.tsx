...
interface PrintRemoteWorkProps {
  data: GeneralRequest[];
  month?: string;
  companyName: string;
  unitName: string;
...
export const PrintRemoteWork = forwardRef<HTMLDivElement, PrintRemoteWorkProps>(
        ...
        >
          <thead>
            <tr>
              ...
              <th style={thStyle}>{t('columns.type')}</th>
              <th style={thStyle}>{t('columns.remoteDate')}</th>
            ...
            {data.map((row, idx) => {
              ...
              return (
                <tr key={idx}>
                  ...
                  <td style={td}>{row.totalHours ? `${Number(row.totalHours).toFixed()}` : '-'}</td>
...
PrintRemoteWork.displayName = 'PrintRemoteWork';
