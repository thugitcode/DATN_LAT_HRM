...
const SHEET_NAME = 'Quản lý làm thêm giờ';
...
const COL_WIDTHS = [22, 16, 22, 16, 12, 12, 14, 28, 22, 14];
...
const buildDataRow = (row: GeneralRequest, t: (key: string) => string): unknown[] => [
  ...
  row.totalHours ? `${Number(row.totalHours).toFixed()} ${t('hour')}` : '-',
  row.reason ?? '-',
...
const buildAoa = (
  ...
  return [
    ...
    buildColHeaderRow(t),
    ...data.map((row) => buildDataRow(row, t)),
    Array(TOTAL_COLS).fill(''),
    Array(TOTAL_COLS).fill(''),
    Array(TOTAL_COLS).fill(''),
    Array(TOTAL_COLS).fill(''),
  ...
  (dataLength: number) => (ws: Parameters<SheetConfig['applyStyles']>[0]) => {
    ...
    applyStyleToRange(ws, (r, c) => {
      ...
      if (r >= ROW_DATA && r <= lastDataRow) {
        const isLeftAlign = c === 0 || c === 2 || c === 7 || c === 8;
...
export const exportOvertimeExcel: ExportFn = ({
  data,
  month,
  companyName,
  unitName,
