...
const SHEET_NAME = 'Đăng ký đào tạo';
...
const COL_WIDTHS = [22, 16, 22, 20, 16, 16, 12, 22, 28, 22, 14];
  ...
  (dataLength: number) => (ws: Parameters<SheetConfig['applyStyles']>[0]) => {
    ...
    applyStyleToRange(ws, (r, c) => {
      ...
      if (r >= ROW_DATA && r <= lastDataRow) {
        const isLeftAlign = c === 0 || c === 2 || c === 7 || c === 8 || c === 9;
...
export const exportTrainingRegistrationExcel: ExportFn = ({
  data,
  month,
  companyName,
  unitName,
