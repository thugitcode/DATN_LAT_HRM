...
export function FileUploadInput({
  ...
  return (
    <div className="space-y-2 w-full">
      {label && (
        <FormLabel isRequired={isRequired} label={label as string} />
      )}
...
interface FormFileUploadInputProps<T extends FieldValues> extends Omit<FileUploadInputProps, 'onFilesSelect' | 'selectedFiles' | 'error' | 'onRemoveFile'> {
  name: Path<T>
  control: Control<T>
  onFilesSelect?: (files: any[]) => void
}

export function FormFileUploadInput<T extends FieldValues>({
  name,
  control,
  onFilesSelect,
  ...props
}: FormFileUploadInputProps<T>) {
  return (
    <Controller
      name={name}
      control={control}
