...
export const ExplanationTable: FC<ExplanationTableProps> = ({
    ...
    if (statusUpper === 'REJECTED') {
      return (
        <Chip
          ...
          startContent={<icons.closeSquare className="rounded-full" width={17} height={17} />}
        >
          Từ chối
        </Chip>
      );
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const selectableRecords = data.filter(
        (r) => r.status.toUpperCase() === 'PENDING' || r.status.toUpperCase() === 'PENDING_HR',
      );
  ...
  const selectableRecordsCount = data.filter(
    (r) => r.status.toUpperCase() === 'PENDING' || r.status.toUpperCase() === 'PENDING_HR',
  ).length;
    ...
    <div className="bg-white rounded-xl overflow-hidden">
                ...
                <div onClick={(e) => e.stopPropagation()}>
                  <Checkbox
                    ...
                    isDisabled={
                      ...
                      record.status.toUpperCase() !== 'PENDING_HR'
                    }
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-[#F4F4F5]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-[#71717A]">trên tổng {total} bản ghi</span>
        </div>
